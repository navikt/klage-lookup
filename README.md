...
It also provides functionality for getting various info from Azure AD, via EntraProxy. This includes checking group membership for a given user, and for the currently logged in user (via token). Also provides info about users in a given enhet.
...
- **Group membership verification**: Checks whether a given user belongs to a specific group in Azure AD via EntraProxy. When checking the currently logged in user, this check is performed on the given token
